...
import pandas as pd
import streamlit as st
import folium # 導入 folium 以便使用 Icon 來自訂顏色
...
if selected_tribe == "清流部落":
    ...
    if qingliu_data.empty:
        ...
        for index, row in qingliu_data.iterrows():
            ...
            # 使用藍色標記 (或預設) 來標示子部落 O_Tribe
            m.add_marker(
                location=(lat, lon), 
                tooltip=o_name, 
                popup=f"📍 子部落：{o_name}",
                # icon=folium.Icon(color='blue', icon='info-sign') # 如果想用特定的 icon
            )
    ...
    # 處理其他部落：只標記單一主要 N_Tribe，使用 GREEN (綠色) 標記
    ...
    # 這裡使用 folium.Icon 來明確指定標記的顏色為 GREEN (綠色)
    green_icon = folium.Icon(color='green', icon='home', prefix='fa')
